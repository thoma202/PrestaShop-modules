...
<script type="text/javascript">
    ...
    // <![CDATA[
    function addShippingDomestic(idPSCarrier, idEbayCarrier, additionalFee, nbSelect) {
        var lastId = 0;
        if ($("#domesticShipping").children('.table').length > 0)
            lastId = parseInt($("#domesticShipping").children('.table').last().attr('data-nb')) + 1;
        var createSelecstShippingDomestic = '';
        createSelecstShippingDomestic += "<table data-nb='"+lastId+"' class='table' style='width:100%;margin-bottom:20px;'><tr>";

        // Carrier Prestashop
        createSelecstShippingDomestic += "<td style='width:150px'><div><select name='psCarrier["+ lastId +"]' class='prestaCarrier'><option value='0' selected>{l s='Select a PrestaShop carrier' mod='ebay'}</option>";
        {foreach from=$newPrestashopZone item=zone}
            {if !empty($zone.carriers)}
                createSelecstShippingDomestic += "<optgroup label='{$zone.name}'>";
                    {foreach from=$zone.carriers item=carrier}
                        createSelecstShippingDomestic += "<option "+ ((typeof(idPSCarrier) != "undefined"  && idPSCarrier == {$carrier.id_carrier})? 'selected="selected"' : '')  +" data-realname='{$zone.name}, {$carrier.name}' value='{$carrier.id_carrier}'>{$carrier.name}</option>";
                    {/foreach}
                createSelecstShippingDomestic += "</optgroup>";
            {/if}
        {/foreach}
        createSelecstShippingDomestic += "</select><span>{l s='Start by choosing a PrestaShop carrier to replicate on eBay' mod='ebay'}</span></div></td>";

        // Carrier eBay
        createSelecstShippingDomestic += "</tr></table>";
        $("#domesticShipping").append(createSelecstShippingDomestic);

        if (typeof(idPSCarrier) != "undefined") {
            var select = $('#domesticShipping').find('.prestaCarrier').eq(nbSelect);
            displayEbayCarrier(select, idEbayCarrier, additionalFee, nbSelect);
        }
    }

    function displayEbayCarrier(select, idEbayCarrier, additionalFee, nbSelect)
    {
        var div = $(select).parent('div');
        var hasA = div.siblings('a').length;
        var tr = div.parent('td').parent('tr');
        var idTable = tr.parent('tbody').parent('table').attr('data-nb');

        if ($(select).val() > 0)
        {
            var stringEbayShipping = '';
            var contentA = $(select).find('option').filter(":selected").attr('data-realname');
            div.fadeOut(400, function(){
                stringEbayShipping += "<p data-validate='{l s='Linked to eBay'}'>{l s='Link' mod='ebay'}<strong> %d </strong>{l s='with eBay carrier' mod='ebay'}</p><div><select name='ebayCarrier[" + idTable + "]' class='eBayCarrier'><option value='0'>{l s='Select eBay carrier' mod='ebay'}</option>";
                {foreach from=$eBayCarrier item=carrier}
                    if('{$carrier.InternationalService}' !== 'true')
                        stringEbayShipping += "<option "+ ((typeof(idEbayCarrier) != "undefined"  && idEbayCarrier == "{$carrier.shippingService}")? 'selected="selected"' : '')  +" value='{$carrier.shippingService}'>{$carrier.description}</option>";
                {/foreach}
                stringEbayShipping += "</select></div>";
                stringEbayShipping = stringEbayShipping.replace('%d', $(select).find('option').filter(":selected").text());
                if (hasA > 0)
                {
                ...
                else
                {
                    ...
                    elementA.on('click', function()
                    {
                        $(this).fadeOut(400, function(){
                            ...
                            tr.children('td').not(":first").remove();
                            tr.parent('tbody').parent('table').css('background-color', '#fff').removeClass('success');
                        });
                        return false;
                    });
                }
                tr.append('<td style="width:150px">').children('td').last().append(stringEbayShipping);

                if (typeof(idEbayCarrier) != "undefined") {
                    var select = $('#domesticShipping').find('.eBayCarrier').eq(nbSelect);
                    processEbayCarrier(select, additionalFee);
                }
            });
        }
        else
    ...
    function processEbayCarrier(select, additionalFee)
    {
        var div = $(select).parent('div');
        var hasA = div.siblings('a').length;
        var tr = div.parent('td').parent('tr');
        ...
        if ($(select).val() != 0)
        {
            ...
            var stringEbayAdditional = "<p>{l s='Additional item cost' mod='ebay'}</p><span>£</span><input "+ ((typeof(additionalFee) != "undefined" && additionalFee > 0) ? 'value="'+additionalFee+'"' : '')  +" name='extrafee["+ idTable +"]' type='text'><span>{l s='Increase the cost when a buyer adds more than one of the same item to their order' mod='ebay'}</span>";
            var atrash = $('<a style="cursor:pointer;">');
            var trash = $('<img>').attr('src', '../img/admin/delete.gif');
            atrash.append(trash)
            ...
            div.fadeOut(400, function(){
                ...
                else
                {
                    ...
                    elementA.on('click', function()
                    {
                        tr.children('td:eq(3), td:eq(2)').remove();
                        $(this).fadeOut(400, function() {
                            $(this).siblings('div').fadeIn();
                        });
                ...
                tr.append('<td style="width:180px">').children('td').last().append(stringEbayAdditional);
                tr.append('<td style="width:1px">').children('td').last().append(atrash);
                div.siblings('p').html(newHtmlP);
                tr.parent('tbody').parent('table').css('background-color', '#DFF2BF').addClass('success');
                if ($("#domesticShipping").children('.table').length == 1)
                {
                    addShippingDomestic();
    ...
    {literal}{{/literal}
        ...
        {foreach from=$existingNationalCarrier item=nationalCarrier key=i}
            addShippingDomestic({$nationalCarrier.ps_carrier}, '{$nationalCarrier.ebay_carrier}', {$nationalCarrier.extra_fee}, {$i});
        {/foreach}
    ...
    {literal}{{/literal}
        ...
        // modif
        $('#domesticShipping').unbind().on('change', 'select', function(){
        ...
        $('#domesticShippingButton').unbind().on('click', function(){
            addShippingDomestic();
            return false;
        })
        // end modif
